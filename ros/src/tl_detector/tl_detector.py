...
class TLDetector(object):
    ...
    def get_closest_waypoint(self, pose):
        ...
        min_dist = 1e100
        index = 0
        for i, pt in enumrate(pose_list):
            dist = math.hypot(pt.pose.pose.position.x-pose.position.x, pt.pose.pose.postion.y-pose.position.y)
            if dist < min_dist:
                min_dist = dist
                index = i
        return index
